...
std::string JSONObject::readFileToString(std::string filename)
{
  char* ret = readFileToChars(filename);
  return string(ret);
}

char* JSONObject::readFileToChars(std::string filename)
{
  char *buffer = NULL;
  size_t size = 0;
...
void JSONObject::showTokens(std::vector <json_token_t*> tokens)
{
...
std::vector<JSONObject::json_token_t*> JSONObject::load(const std::string file_path)
{
...
std::vector<JSONObject::json_token_t*> JSONObject::parse(const std::string json_string)
{
			...
			case '}': case ']':
      {
        json_type_t type;
          ...
          return this->tokens;
				...
				while (1) {
          // printf("token next_token=%d, start:%d end:%d parent=%d\n",parser.next_token, token->start, token->end, token->parent_token);
...
void JSONObject::assignToken(json_parser *parser, json_token_t *token)
{
  parser->next_token++;
...
int JSONObject::parseNumbersBools(json_parser *parser, std::string json_string)
{
  json_token_t *token = new json_token_t;
	int cutout = 0;
...
int JSONObject::parseString(json_parser *parser, std::string json_string)
{
  json_token_t *token = new json_token_t;
	int start = parser->pos;
	parser->pos++;
...
std::string JSONObject::getTokenTypeLabel(json_type_t type)
{
  int t = type;
  switch(t){
    case JSON_TYPE_NONE:
...
std::vector<JSONObject::json_token_t*> JSONObject::getChildrenForToken(json_token_t* parent)
{
  std::vector <JSONObject::json_token_t*> children;
...
JSONObject::json_token_t* JSONObject::getChildForToken(json_token_t* parent, std::string key) // returns the first child for key.
{
  JSONObject::json_token_t* child = new json_token_t();
...
std::string JSONObject::getValueForToken(json_token_t* parent)
{
  if (parent->start >= 0){
...
std::string JSONObject::getChildValueForToken(json_token_t* parent, std::string key)
{
  JSONObject::json_token_t* child_token = getChildForToken(parent, key);
...
std::vector<JSONObject::json_token_t*> JSONObject::getArrayForToken(json_token_t* parent, std::string key) // returns the first child for key.
{
  std::vector <json_token_t*> tokens = this->tokens;
