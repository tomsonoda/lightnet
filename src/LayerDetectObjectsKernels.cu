...
__global__ void calcDetectObjectsBackwardGPU( float *dz_next_layer, float *dz_in, float *dz, float *in, int max_bounding_boxes, int max_classes )
{
  int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
	...
	for( int i = 0; i < max_bounding_boxes; i=i+(4+max_classes)){
		...
		dz[index  ] = activator_derivative( in( b, i  , 0, 0 ) ) * dz_in( b, i  , 0, 0 ); // x: sigmoid derivative * grads
		dz[index+1] = activator_derivative( in( b, i+1 , 0, 0 ) ) * dz_in( b, i+1, 0, 0 ); // y: sigmoid derivative * grads
...
void detectObjectsBackwardGPU( float *dz_in, float *dz, float *in, int batch_size, int in_size_x, int in_size_y, int in_size_z, int max_bounding_boxes, int max_classes );
{
  CudaObject cuda = CudaObject();
  dim3 grid = cuda.cudaGridSize(batch_size);
  calcDetectObjectsBackwardGPU<<<grid, BLOCK>>>( dz_next_layer, dz_in, dz, in, max_bounding_boxes, max_classes );
}
